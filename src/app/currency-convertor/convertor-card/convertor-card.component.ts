...
export class ConvertorCardComponent implements OnInit {
  ...
  public convertorForm: FormGroup = this.fb.group({
    ...
    // eslint-disable-next-line no-sparse-arrays
    targetAmount: [, [Validators.pattern(/^\d*(\.\d*)?$/)]],
    // eslint-disable-next-line no-sparse-arrays
    ...
    if (!this.supportedCurrencies) {
      this.utilityService.getAllSupportedCurrencies().subscribe(
        (response: Currency[]) => {
          ...
          const sourceCurrencyIndex = this.findIndexOfCurrency(fixedSourceCurrency, this.supportedCurrencies)
          const targetCurrencyIndex = this.findIndexOfCurrency(fixedTargetCurrency, this.supportedCurrencies)
  ...
  public findIndexOfCurrency(currency: Currency, currencyArray: Currency[]): number {
    return currencyArray.map(value => value.currencySymbol).indexOf(currency.currencySymbol)
  }
  ...
  public convertCurrency() {
    ...
    const inputCurrency = {
      ...
      sourceCurrency: this.convertorForm.get('sourceCurrency').value.currencySymbol,
      targetCurrency: this.convertorForm.get('targetCurrency').value.currencySymbol,
    }

    this.utilityService.convertCurrency(inputCurrency).subscribe(
      (result: ConvertedCurrency) => {
        this.convertorForm.patchValue({
          targetAmount: result.targetAmount.toFixed(10),
        })
  ...
  public updateCurrency() {
    const sourceCurrency: Currency = this.convertorForm.get('sourceCurrency').value
    const targetCurrency: Currency = this.convertorForm.get('targetCurrency').value
    this.conversionResult = undefined
    this.enableConvertButtonWithText(this.getCurrencyText(sourceCurrency.currencySymbol, targetCurrency.currencySymbol))
  }
